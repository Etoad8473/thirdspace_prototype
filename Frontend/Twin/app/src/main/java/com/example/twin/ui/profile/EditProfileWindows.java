...
import com.example.twin.api.SlimCallBack;
import com.example.twin.app.AppController;
import com.example.twin.model.Profile;
import com.example.twin.utils.Const;
...
public class EditProfileWindows extends AppCompatActivity implements View.OnClickListener {
    ...
    public void postObj(){
        ...
        newProfile.setPassword("jhjk");
        newProfile.setUsername("hayday");
        //newTrivia.setPersonalityId(3);
        //newTrivia.setEventId(0);
        GetProfileApi().PostProfileByBody(newProfile).enqueue(new SlimCallBack<Profile>(trivia->{
    ...
    void RegenerateAllTriviasOnScreen( TextView apiText1){
        ...
        GetProfileApi().GetAllProfile().enqueue(new SlimCallBack<List<Profile>>(profiles ->{
            apiText1.setText("");

            for (int i = profiles.size()-1; i>= 0; i--){
                apiText1.append(profiles.get(i).printable());
            }
        }, "GetAllProfile"));
